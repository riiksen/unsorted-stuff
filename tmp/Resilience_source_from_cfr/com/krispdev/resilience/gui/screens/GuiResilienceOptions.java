...
public class GuiResilienceOptions
extends GuiScreen {
    ...
    public void drawScreen(int i, int j, float f) {
        GuiResilienceOptions.drawRect(0, 0, Resilience.getInstance().getInvoker().getWidth(), Resilience.getInstance().getInvoker().getHeight(), -14671840);
        for (CheckBox box : CheckBox.checkBoxList) {
            box.draw(i, j);
        }
        Resilience.getInstance().getPanelTitleFont().drawCenteredString("Client options:", Resilience.getInstance().getInvoker().getWidth() / 2, 8.0f, -16755201);
